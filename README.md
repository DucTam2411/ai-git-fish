...
| `aibpm` | BPM (Anymateme Web platform, fixed project) sprint board: pick a sprint, browse its tasks grouped by assignee. `</>` cycles the highlighted task's status instantly (redraws from local state, real update fires in the background — no network wait per keypress); `ctrl-r` force-refreshes from Leantime; `enter` on a task opens the full status picker, `enter` on `➕ New task` fzf-picks an assignee and creates one — all loop back to the board until you exit. |
...
aibpm                                     # pick sprint -> board by assignee -> </> cycle status, ctrl-r refresh, enter to act
